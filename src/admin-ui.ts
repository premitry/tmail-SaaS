...
function saveBtn(fn){ return '<button onclick="window.__sb=this;'+fn+'" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg mt-1">Simpan</button>'; }
function saved(){ var b=window.__sb; if(!b||!b.parentNode){ toast('Tersimpan'); return; } var m=b.__savedMsg; if(!m){ m=document.createElement('span'); m.className='text-green-600 dark:text-green-400 text-sm font-medium ml-2 whitespace-nowrap'; m.innerHTML='<i class="fas fa-check-circle mr-1"></i>Tersimpan'; b.insertAdjacentElement('afterend',m); b.__savedMsg=m; } m.style.transition=''; m.style.opacity='1'; m.style.display='inline'; clearTimeout(b.__savedT); b.__savedT=setTimeout(function(){ m.style.transition='opacity .4s'; m.style.opacity='0'; setTimeout(function(){ m.style.display='none'; },400); },2500); }
...
async function vSettings(){
  ...
  function imgField(kind,label,desc){ const cur=window.__img[kind]; const shown=cur||(kind==='favicon'?'/favicon.svg':window.DEFAULT_LOGO);
  ...
  card('General','Identitas & warna tampilan web publik.',
    ...
    saveBtn('saveGeneral()'))+
  card('Terima Email','Cara sistem menerima email masuk untuk domain-domainmu.',
    // Toggle mode
    (function(){ var mode = s.imap_host ? 'imap' : 'worker'; window.__mailMode = mode;
      ...
      return '<div class="flex flex-col md:flex-row gap-3 mb-1">'+opt('worker','Otomatis (Rekomendasi)','Terima email langsung tanpa perlu server sendiri.')+opt('imap','IMAP','Kalau kamu punya mailbox catch-all sendiri.')+'</div>';
    })()+
    ...
    '<div id="modeWorker" style="display:'+(s.imap_host?'none':'block')+'" class="text-sm space-y-2">'+
      (function(){ var t=s.last_worker_email_at||0; var ago=t?Math.floor((Date.now()-t)/60000):0;
        var badge = t
          ? '<div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium" style="background:#dcfce7;color:#166534"><i class="fas fa-check-circle"></i> Penerimaan aktif · email terakhir '+(ago<1?'baru saja':ago<60?ago+' menit lalu':ago<1440?Math.floor(ago/60)+' jam lalu':Math.floor(ago/1440)+' hari lalu')+'</div>'
          : '<div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium" style="background:#fee2e2;color:#991b1b"><i class="fas fa-exclamation-circle"></i> Belum aktif · ikuti langkah di bawah</div>';
        return badge;
      })()+
      '<div class="text-gray-600 dark:text-gray-300">Teruskan (forward) semua email domainmu ke alamat ini: <b class="font-mono select-all" style="background:rgba(148,163,184,.15);padding:2px 6px;border-radius:4px">catchall@imapku.icu</b></div>'+
    '</div>'+
    // === Panel IMAP ===
...
async function put(patch){ const r=await api('/settings',{method:'POST',body:JSON.stringify(patch)}); if(r.error)alert(r.error); else { saved(); if(r.settings)window.__S=r.settings; } }
...
function clearImg(kind){ window.__img[kind]=''; const p=$('#'+kind+'Prev'); if(p)p.src=(kind==='favicon'?'/favicon.svg':window.DEFAULT_LOGO); const n=$('#'+kind+'Note'); if(n){ n.className='text-xs text-gray-400'; n.innerHTML='bawaan TMail · <button type="button" onclick="clearImg(\''+kind+'\')" class="text-red-600 underline">pakai bawaan</button>'; } }
